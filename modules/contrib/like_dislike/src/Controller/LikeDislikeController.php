...
class LikeDislikeController extends ControllerBase {
  ...
  public function handler($clicked, $data) {
    ...
    if ($user == 0) {
      ...
      $user_login_register = $this->likeDislikeLoginRegister();
    ...
    if ($clicked == 'like') {
      if (!$already_clicked) {
        ...
        return $this->likeDislikeStatus($response, $decode_data->entity_id);
      }
      $return = $response->addCommand(
        new HtmlCommand('#like-' . $decode_data->entity_id, (string) $entity_data->$field_name->likes)
      );
    }
    elseif ($clicked == 'dislike') {
      ...
      $return = $response->addCommand(
        new HtmlCommand('#dislike-' . $decode_data->entity_id, (string) $entity_data->$field_name->dislikes)
      );
    }
   ...
   *
   * @return mixed
   *   return render outputx
   */
  protected function likeDislikeLoginegister() {
    $options = [
      'attributes' => [
        'class' => [
          'use-ajax',
   ...
   */
  protected function likeDislikeStatus(AjaxResponse $response, $entity_id) {
    return $response->addCommand(
